...
module.exports = (router) => {
  router.patch('/:collection',
    ...
    .queryParam('mergeObjects', joi.boolean().default(true), 'Whether to merge objects instead of replacing them.' +
      ' Default `true`')
    .body(reqBodySchema, 'The JSON object/s to persist.')
    .response(200, ['application/json'], 'The update call succeeded. For multiple docs, this is returned only if' +
      ' **ALL** documents were successfully updated.')
    ...
    .error(412, 'One or more documents could not be updated (due to a conflict or some other failed pre-condition).' +
  ...
  console.log('Loaded "update" route');
};
