...
const {
  updateSingle,
  updateMultiple
} = require('../../handlers/updateHandlers')
const {
  DB_OPS,
  DOC_ID_REGEX,
  COLL_NAME_REGEX,
...
module.exports = router => {
  router
    ...
    .queryParam(
      'keepNull',
      joi.boolean().default(true),
      'Whether to keep keys with null values. Default `true`'
    )
    .queryParam(
      'mergeObjects',
      joi.boolean().default(true),
    ...
    .response(
      200,
      ['application/json'],
      'The update call succeeded. For multiple docs, this is returned only if' +
    ...
    .error(
      412,
      'One or more documents could not be updated (due to a conflict or some other failed pre-condition).' +
