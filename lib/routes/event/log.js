...
module.exports = router => {
  ...
  buildEndpoint(router.get('/log', processLogRequest, 'logGet'))
    .queryParam('path', pathSchema, 'The path pattern to pick nodes whose logs should be returned.')
...
function buildEndpoint (endpoint) {
  return endpoint
    ...
    .queryParam('groupSort', joiCG.string().valid('asc', 'desc').optional(), dd`
    ...
    .queryParam('groupSkip', joiCG.number().integer().min(0).optional(), dd`
    ...
    .queryParam('groupLimit', joiCG.number().integer().min(0).optional(), dd`
