...
module.exports = router => {
  buildEndpoint(router.post('/traverse', processTraverseRequest, 'traversePost'))
    ...
    .summary('Traverse historic graph states.')
...
function buildEndpoint (endpoint) {
  return endpoint
    .queryParam('timestamp', JoiRG.number(), dd`
      The unix timestamp (sec) for which to traverse node states. Precision: 0.1μs.
