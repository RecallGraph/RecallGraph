...
const traverseBaseSchema = JoiRG.object().keys({
...
const TRAVERSE_BODY_SCHEMA = traverseBaseSchema.concat(JoiRG.object().keys({
  pFilter: JoiRG.string().filter().empty('')
}))
const KSP_BODY_SCHEMA = traverseBaseSchema.concat(JoiRG.object().keys({
