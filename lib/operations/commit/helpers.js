...
const {
  cloneDeep, pick, memoize, mapKeys, intersection, noop, filter, last, difference, values, chain, find
} = require('lodash')
...
function getNodeBraceScope (collections) {
  return {
    ...
    filters: (searchPattern, collection) => {
      ...
      return {
        existing: aql`filter v._key in ${keys}`,
...
const getTransientOrCreateLatestSnapshot = attachSpan(
  function getTransientOrCreateLatestSnapshot (collName, lastEvtNode, node, ctime) {
    ...
    const prevSSid = lastEvtNode['last-snapshot']

    let ssNode = {
      _id: prevSSid
    }
    let hopsFromLast = lastEvtNode['hops-from-last-snapshot'] + 1

    if (ssInterval > 0 && hopsFromLast > ssInterval) {
      ssNode = {
        ctime,
        data: node
      }
