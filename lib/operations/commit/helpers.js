    ...
    updated: (collName, key, evtNode) => {
      ...
      if (endpoints.length) {
        ...
        if (evtNode.meta.fromNew) {
          let fromParts = evtNode.meta.fromNew.split('/')
          let fCollName = fromParts[0]
          let fKey = fromParts[1]
          let fromKey = `${fCollName}.${fKey}`
          fromNew = `${skCollMap[fCollName]}/${fromKey}`

          fromParts = evtNode.meta.fromOld.split('/')
          fCollName = fromParts[0]
          fKey = fromParts[1]
          fromKey = `${fCollName}.${fKey}`
          fromOld = `${skCollMap[fCollName]}/${fromKey}`
        }
        const from = filter([fromOld, fromNew])

        if (evtNode.meta.toNew) {
          let toParts = evtNode.meta.toNew.split('/')
          let tCollName = toParts[0]
          let tKey = toParts[1]
          let toKey = `${tCollName}.${tKey}`
          toNew = `${skCollMap[tCollName]}/${toKey}`

          toParts = evtNode.meta.toOld.split('/')
          tCollName = toParts[0]
          tKey = toParts[1]
          toKey = `${tCollName}.${tKey}`
          toOld = `${skCollMap[tCollName]}/${toKey}`
        }
...
exports.prepRestore = attachSpan(function prepRestore (collName, node) {
  const coll = db._collection(collName)
  const result = coll.insert(node, {
    returnNew: true
  })
...
exports.prepReplace = attachSpan(function prepReplace (collName, node, { ignoreRevs = true } = {}) {
  ...
  const dbNode = coll.document(selector)

  const prevEvent = getLatestEvent(dbNode, coll)
  if (prevEvent['is-origin-node']) {
    const e = new Error(
      `Event log not found for node with _id: ${dbNode._id}. Run \`sync\` first to make the event log catch up.`
    )
  ...
  const result = coll.replace(selector, node, {
    returnNew: true,
    returnOld: true
  })
  const time = dbtime()
...
exports.prepRemove = attachSpan(function prepRemove (collName, node, { ignoreRevs = true } = {}) {
  ...
  const result = coll.remove(selector, {
    returnOld: true
  })
  const time = dbtime()
  result.new = {}
