...
const { merge, cloneDeep, pick, once, memoize } = require('lodash')
...
const ensureEventSupernode = once(() => {
...
function getTransientOrCreateLatestSnapshot (
  ...
  let ssNode = null; let hopsFromLast; let prevSSid; let hopsTillNext
  if (ssInterval > 0) {
    if (
...
const ensureSnapshotOriginNode = memoize(collName => {
...
const ensureEventOriginNode = memoize(collName => {
