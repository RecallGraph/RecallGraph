...
const { merge, cloneDeep, pick, memoize, mapKeys, intersection } = require('lodash')
...
exports.insertEventNode = function insertEventNode (nodeMeta, time, event, ssData, prevEvent, collName) {
  const evtNode = {
...
exports.prepUpdate = function prepUpdate (collName, node, { keepNull = true, mergeObjects = true, ignoreRevs = true } = {}) {
  const selectorKeys = ['_key', '_id']
  if (!ignoreRevs) {
    selectorKeys.push('_rev')
  }
...
exports.updateSkeletonGraph = function updateSkeletonGraph (evtNode) {
