...
function getShowQueryFilters (timestamp) {
  return aql`
    ...
    filter v.event != 'deleted'
    filter overshot or undershot or (
        vIsPrior and (v.meta._rev == document(coll, v.meta._id)._rev)
    )
  `
}
...
exports.getSortingClause = function getSortingClause (sort, groupBy, countsOnly) {
  ...
  const groupExpr = getGroupExpr(groupBy)
  const sortPrefix = sortDir && (groupExpr || !countsOnly) ? 'sort ' : ''
  const primarySort = !sortPrefix ? '' : (countsOnly ? 'total' : groupExpr ? groupBy : 'v.meta._id') + ` ${sortDir}`
  const secondarySort = primarySort && groupBy && countsOnly ? `, ${groupBy} ${sortDir}` : ''
