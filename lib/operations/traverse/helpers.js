...
function addVisit (traversal, minDepth, v, e, currentDepth, currentPath, pFilterFn) {
  const path = cloneDeep(currentPath)
  if (e !== null) {
    path.edges.push(omit(e.data(), 'id'))
    path.eSet.add(e.id())
  }
  path.vertices.push(omit(v.data(), 'id'))
  path.vSet.add(v.id())

  if (currentDepth >= minDepth && pFilterFn(path)) {
...
function depthFirstWalk (traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, v,
  e = null, currentDepth = 0,
  ...
  if (visitIsValid(traversal, uniqueVertices, uniqueEdges, edgeCollections, v, e, currentPath)) {
    const path = addVisit(traversal, minDepth, v, e, currentDepth, currentPath, pFilterFn)
    ...
    if (currentDepth < maxDepth) {
      ...
      for (const key in nbd) {
        ...
        for (const ne of group.edges) {
          depthFirstWalk(traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, nv,
            ne, depth, path)
        }
      }
    }
...
function breadthFirstWalk (traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, nbds,
  currentDepth = 0) {
  const childNbds = []
  ...
  if (currentDepth < maxDepth) {
    breadthFirstWalk(traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, childNbds,
      currentDepth + 1)
  }
}
...
function buildFilteredGraph (svid, vertices, edges, minDepth, maxDepth, bfs, uniqueVertices, uniqueEdges,
  ...
  if (bfs) {
    breadthFirstWalk(traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, [
      {
    ...
    depthFirstWalk(traversal, minDepth, maxDepth, uniqueVertices, uniqueEdges, edgeCollections, pFilterFn, sv)
  }
...
function buildNodeGroupsByType (timestamp, types, vFilter, eFilter) {
  const built = {
    vertices: [],
    edges: []
  }
