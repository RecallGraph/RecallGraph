...
const { getCollectionType, getComponentTagOption } = require('../helpers')
const {
...
function getNonServiceCollections () {
  return difference(
    db._collections()
      .map(coll => coll.name())
      .filter(collName => !collName.startsWith('_')),
...
function getGraphScope () {
  return {
    ...
    filters: searchPattern => {
      ...
      return {
        filter: aql`filter p.vertices[1]['collection'] in ${collNames}`,
        prune: aql`length(p.edges) == 1 && v['collection'] not in ${collNames}`
      }
    }
  }
}
...
function getCollectionScope (collections) {
  return {
    ...
    filters: searchPattern => {
      ...
      return {
        filter: aql`filter p.vertices[1]['collection'] in ${matches}`,
        prune: aql`length(p.edges) == 1 && v['collection'] not in ${matches}`
      }
    }
  }
}
  ...
  return {
    ...
    filters: searchPattern => {
      ...
      return {
        ...
        prune: aql`
            length(p.edges) == 2 && (p.vertices[1]['collection'] not in ${collMatches}
