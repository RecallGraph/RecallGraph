...
const { difference, values, find, fromPairs, chain, isString, concat } = require('lodash');
...
function getGraphCollectionNamesMap(graphNames) {
  return chain(graphNames)
    .map(name => {
      ...
      const collections = concat(graph._vertexCollections(), graph._edgeCollections(), graph._orphanCollections());
...
function getNodeGlobScope(collections) {
  return {
    pathPattern: '/ng/**',
    prefix: '/ng/',
    filters: (searchPattern) => {
      const collPattern = searchPattern.split('/')[0];
      const collMatches = minimatch.match(collections, collPattern);
      const idPattern = minimatch.makeRe(searchPattern);

      return [
        aql`filter p.vertices[1]['origin-for'] in ${collMatches}`,
        aql`filter regex_test(p.vertices[2].meta._id, ${idPattern})`
      ];
    }
  };
}
...
function getNodeBraceScope(collections) {
  return {
    ...
    filters: (searchPattern) => {
      const collPattern = searchPattern.split('/')[0];
      const collMatches = chain(expand(collPattern)).intersection(collections).uniq().value();
      ...
      return [
        aql`
          ...
          `,
        aql`filter p.edges[1].meta._key in keyGroups[collName]` //See initializers below for keyGroups definition.
      ];
    },
    initializers: (searchPattern) => {
      const collPattern = searchPattern.split('/')[0];
      const collMatches = chain(expand(collPattern)).intersection(collections).uniq().value();
      const idMatches = expand(searchPattern);
      const keyGroups = chain(idMatches)
...
function getAvailableScopes(collections) {
  return {
    ...
    nodeRegex: getNodeGlobScope(collections),
...
exports.getLimitClause = function getLimitClause(skip, limit) {
  if (limit) {
