...
const { FoxxTracer, reporters: { DatadogReporter } } = require('foxx-tracing')

// const tracer = new FoxxTracer(new ConsoleReporter())
const ddURL = module.context.service.configuration['dd-agent-url']
const tracer = new FoxxTracer(new DatadogReporter(ddURL, 'rglocal'))
...
exports.snapshotInterval = function snapshotInterval (collName) {
  const snapshotIntervals = module.context.service.configuration['snapshot-intervals']
...
exports.startSpan = function startSpan (name, options = {}, traceOverride) {
...
exports.endSpan = function endSpan (span) {
  span.finish()
