...
const SERVICE_COLLECTIONS = Object.freeze({
  ...
  skeletonEdges: module.context.collectionName(
    configuration['skeleton-edge-coll-suffix']
  )
})
...
exports.hash = function hash (input, radix) {
  radix = Math.abs(parseInt(radix)) || 1
  if (!isString(input)) {
    input = ''
  }

  if ((radix === 1) || (input.length === 0)) {
    return 0
  }
